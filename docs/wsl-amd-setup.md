...
接続先の既定値は `client/.env.local` で指定できます。

```env
VITE_GIC_DEFAULT_BASE_URL=http://<desktop-pc-lan-ip>:8000
```

`.env.local` を変更した場合は `pnpm dev` を再起動してください。
...
```text
IPv4 アドレス       : 192.168.0.10  <- MacBookから指定する入口
デフォルト ゲートウェイ: 192.168.0.1  <- これは使わない
```
...
```powershell
.\scripts\windows\refresh-wsl-portproxy.ps1 -LanIp 192.168.0.10
```
...
curl.exe http://192.168.0.10:8000/api/health
```
...
```powershell
.\scripts\windows\check-lan-portproxy.ps1 -LanIp 192.168.0.10
```
